...
must_haves:
  truths:
    ...
    - "DataQueryService.schema_ddl returns enhanced DDL (already calls generate_schema_ddl, which is enhanced in 12-02)"
...
**Note:** DataQueryService already calls `generate_schema_ddl()` via the `schema_ddl` property (line 77).
After Plan 12-02 enhances that function, DataQueryService will automatically use the enriched DDL.
This task adds source attribution to complement the enhanced DDL.
...
3. Verify DataQueryService uses enhanced DDL:
   ```python
   python -c "
   from jobforge.api.data_query import DataQueryService
   from jobforge.pipeline.config import PipelineConfig
   svc = DataQueryService(PipelineConfig())
   ddl = svc.schema_ddl
   print('DDL has comments:', 'COMMENT' in ddl)
   print('DDL has relationships:', 'RELATIONSHIPS' in ddl)
   assert 'COMMENT' in ddl, 'DataQueryService should use enhanced DDL'
   "
   ```

4. Test Orbit retriever DDL:
   ```python
   python -c "
...
- [ ] DataQueryService.schema_ddl returns enhanced DDL with COMMENT clauses (via generate_schema_ddl)
