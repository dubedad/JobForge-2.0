...
Note: The MCP permissions enable Claude Code (running in VS Code Pro) to interact with Power BI Desktop via the Power BI MCP Server during live demos. These permissions are used by the EXTERNAL Claude Code session, not by the Python backend.
  </action>
  <verify>
...
Demo flow:
1. User opens JobForge web UI in browser (served by `jobforge demo`)
2. User clicks "Load" -> SSE stream starts, showing what will be built
3. User ALSO has Claude Code open in VS Code Pro, runs `/stagegold`
4. Claude Code (with MCP permissions) executes the actual deployment to Power BI
5. The web UI narration and Claude Code execution happen in parallel
6. User manually arranges windows (browser + Power BI Desktop side-by-side)
...
2. Update `src/jobforge/cli/commands.py`:
   - Add `demo` command using Typer:
     ...
     def demo(
         ...
         1. Start this server: jobforge demo
         2. Open browser to http://localhost:8080
         3. In VS Code Pro, run /stagegold to trigger actual deployment
...
6. `curl http://localhost:8080/api/deploy/stream` returns SSE event stream (narration events)
7. Verify DemoOrchestrator does NOT import MCPClient (narration only, no deployment execution)
</verification>
